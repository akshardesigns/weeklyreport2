...
async function getActualHeaders(sheets) {
  ...
  const existingLower = row.map((h) => String(h || '').trim().toLowerCase());
  const missing = HEADERS.filter((h) => !existingLower.includes(h.toLowerCase()));
  if (missing.length > 0) {
    const updated = [...row, ...missing];
    ...
    return updated.map((h) => String(h || '').trim());
  }
...
function objToRow(obj, actualHeaders) {
  return actualHeaders.map((h) => (obj[h] !== undefined && obj[h] !== null ? obj[h] : ''));
}
